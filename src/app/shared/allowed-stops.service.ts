...
export class AllowedStopService {
  private rideAllowedStopsUrl = 'api/rideAllowedStops';
  httpOptions = {
  ...
  getAllowedStops(): Observable<AllowedStops[]> {
    return this.http.get<AllowedStops[]>(this.rideAllowedStopsUrl).pipe(
      tap((_) => this.logService.info('fetched allowed stops')),
      catchError(this.handleError<AllowedStops[]>('getAllowedStops', []))
    );
  }
